...
async function run (res:any,req:any){
    ...
    log.log(`Handling ${type} report`);
    try{
        await handlers[type](parsedFile,name,company,log);
        return res.status(200).json(JSON.stringify({text: 'Success'}))
    }catch (e) {
        return res.status(500).json(JSON.stringify({text: 'Error'}))
    }
